...
size_t ReadUByteDataset(const char *image_filename,
                        const char *label_filename,
                        uint8_t *data,
                        uint8_t *labels,
                        size_t *width,
                        size_t *height) {
    assert(("width is nullptr", width));
    assert(("height is nullptr", height));
    ...
    if (image_header.length != label_header.length) {
        printf(
            "ERROR: Dataset file mismatch (number of images does not match the "
            "number of labels)\n");
        fclose(imfp);
        fclose(lbfp);
        return 0;
    ...
    *height = image_header.height;
    ...
    if (data != nullptr) {
        if (fread(data,
                  sizeof(uint8_t),
                  image_header.length * *width * *height,
                  imfp) != image_header.length * *width * *height) {
    ...
    if (labels != nullptr) {
        if (fread(labels, sizeof(uint8_t), label_header.length, lbfp) !=
            label_header.length) {
